...
import { Sparkles, Brain, Target, FileText, Wand2, Check, X, Copy, RefreshCw, Loader2, Eye } from "lucide-react";
...
export function AISummaryPage({ cvData, setCVData }: AISummaryPageProps) {
  const [jobRole, setJobRole] = useState("");
  const [jobDesc, setJobDesc] = useState("");
  const [generating, setGenerating] = useState(false);
  const [generated, setGenerated] = useState(cvData.aiSummary);
  ...
  const generate = () => {
    ...
    setTimeout(() => {
      ...
      setCVData({ ...cvData, aiSummary: summary });
      setGenerating(false);
    }, 2500);
  };
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          <h3 className="font-semibold text-gray-800 mb-1 flex items-center gap-2">
            <Brain size={15} className="text-violet-500" />Generate with AI
          </h3>
          <p className="text-xs text-gray-400 mb-5">Tell the AI about your target role for a personalized summary</p>

          <div className="flex flex-col gap-4">
            <Field label="Target Job Role" required>
              <Input value={jobRole} onChange={setJobRole} placeholder="e.g. Senior Frontend Developer" icon={<Target size={14} />} />
            </Field>
            <Field label="Job Description" hint="Paste the job description to get a perfectly tailored summary">
              <Textarea value={jobDesc} onChange={setJobDesc} placeholder="Paste the job description here for best results…" rows={6} />
            </Field>
            <div className="bg-violet-50 border border-violet-200 rounded-xl p-4">
              <p className="text-xs font-semibold text-violet-800 mb-1 flex items-center gap-1">
                <Sparkles size={12} />AI will use your CV data
              </p>
              <p className="text-xs text-violet-600">
                The AI will analyze your experience, skills, and education to craft a personalized summary.
              </p>
            </div>
            <Btn onClick={generate} disabled={generating || !jobRole}>
              {generating ? (
                <><Loader2 size={16} className="animate-spin" />Generating your summary…</>
              ) : (
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                <FileText size={15} className="text-violet-500" />Generated Summary
              </h3>
              {generated && (
                <div className="flex gap-2">
                  <Btn size="sm" variant="ghost" onClick={() => navigator.clipboard?.writeText(generated)}>
                    <Copy size={12} />Copy
                  </Btn>
                  <Btn size="sm" variant="secondary" onClick={generate} disabled={generating}>
            ...
            {generating ? (
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                  <Btn size="sm" onClick={() => setCVData({ ...cvData, aiSummary: generated })}>
                    <Check size={13} />Use this summary
                  </Btn>
          ...
          {cvData.aiSummary && (
            <Card className="p-6">
              <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                <Eye size={15} className="text-violet-500" />Current Summary in CV
              </h3>
              <div className="border-t-2 border-violet-600 pt-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{cvData.aiSummary}</p>
              </div>
            </Card>
          )}
        </div>
