...
export function DownloadCVPage({ cvData, onNav }: DownloadCVPageProps) {
  ...
  const fileName = `${cvData.personalInfo.firstName || "Your"}_${cvData.personalInfo.lastName || "CV"}_CV.pdf`;
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Template: <span className="font-medium capitalize">{cvData.selectedTemplate}</span> · Ready to export
                </p>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <p className="text-sm text-blue-800 font-semibold mb-1 flex items-center gap-2">
                <Info size={14} />ATS Compatibility
              </p>
              <p className="text-xs text-blue-700">
                Your selected template is ATS-compatible and passes major applicant tracking systems.
              </p>
            </div>

            <Btn size="lg" onClick={handleDownload} disabled={downloading} className="w-full">
              {downloading ? (
            ...
            <Btn variant="outline" onClick={() => onNav("preview-cv")} className="w-full">
              <Eye size={15} />Preview before downloading
            </Btn>
          </div>
        )}
      </Card>
